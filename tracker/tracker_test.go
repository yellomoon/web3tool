...
func testFilter(t *testing.T, provider Provider, filterConfig *FilterConfig) []*web3tool.Log {
...
func TestFilterIntegration(t *testing.T) {
	...
	// filter by address
	logs := testFilter(t, client.Eth(), &FilterConfig{Address: []web3tool.Address{addr0}})
	...
	logs = testFilter(t, client.Eth(), &FilterConfig{Topics: [][]*web3tool.Hash{nil, {&topic}}})
	require.NotEmpty(t, logs)
}
...
func testTrackerSyncerRandom(t *testing.T, n int, backlog uint64) {
	...
	for i := 0; i < n; i++ {
		...
		var added, removed []*web3tool.Log
		for {
			select {
			case evnt := <-tt.EventCh:
...
func (m *mockClientWithLimit) GetLogs(filter *web3tool.LogFilter) ([]*web3tool.Log, error) {
