...
import (
	...
	"github.com/yellomoon/web3tool/blocktracker"
	"github.com/yellomoon/web3tool/etherscan"
	"github.com/yellomoon/web3tool/jsonrpc/codec"
	"github.com/yellomoon/web3tool/tracker/store"
	"github.com/yellomoon/web3tool/tracker/store/inmem"
)
...
type FilterConfig struct {
	Address []web3tool.Address `json:"address"`
	Topics  [][]*web3tool.Hash `json:"topics"`
	Start   uint64
	Hash    string
	Async   bool
}
...
func (f *FilterConfig) getFilterSearch() *web3tool.LogFilter {
...
type Provider interface {
	...
	GetBlockByHash(hash web3tool.Hash, full bool) (*web3tool.Block, error)
	GetBlockByNumber(i web3tool.BlockNumber, full bool) (*web3tool.Block, error)
	GetLogs(filter *web3tool.LogFilter) ([]*web3tool.Log, error)
...
func (t *Tracker) GetLastBlock() (*web3tool.Block, error) {
...
func (t *Tracker) storeLastBlock(b *web3tool.Block) error {
	if b.Difficulty == nil {
...
func (t *Tracker) findAncestor(block, pivot *web3tool.Block) (uint64, error) {
...
func (t *Tracker) emitLogs(typ EventType, logs []*web3tool.Log) {
	evnt := &Event{}
	if typ == EventAdd {
		evnt.Added = logs
	}
...
func (t *Tracker) fastTrack(filterConfig *FilterConfig) (*web3tool.Block, error) {
	...
	if t.config.EtherscanAPIKey != "" {
		...
		e, err := etherscan.NewEtherscanFromNetwork(web3tool.Network(chainID.Uint64()), t.config.EtherscanAPIKey)
		if err != nil {
			// there is no etherscan api for this specific chainid
			return nil, nil
		}

		getAddress := func(addr web3tool.Address) (uint64, error) {
...
func (t *Tracker) removeLogs(number uint64, hash *web3tool.Hash) ([]*web3tool.Log, error) {
	...
	var remove []*web3tool.Log
	for {
		elemIndex := index - 1

		var log web3tool.Log
...
func revertLogs(in []*web3tool.Log) (out []*web3tool.Log) {
...
func (t *Tracker) doFilter(added []*web3tool.Block, removed []*web3tool.Block) (*Event, error) {
	evnt := &Event{}
	if len(removed) != 0 {
		pivot := removed[0]
...
func (t *Tracker) getBlockByNumber(blockNumber uint64) (*web3tool.Block, error) {
	block, err := t.provider.GetBlockByNumber(web3tool.BlockNumber(blockNumber), false)
	if err != nil {
		return nil, err
	} else if block == nil {
		// if block does not exist (for example reorg happened) GetBlockByNumber will return nil, nil
