...
func testMultipleStores(t *testing.T, setup SetupDB) {
	...
	if err := entry0.StoreLogs([]*web3tool.Log{&log}); err != nil {
		t.Fatal(err)
	}
	...
	if err := entry1.StoreLogs([]*web3tool.Log{&log}); err != nil {
		t.Fatal(err)
	}
...
func testStoreLogs(t *testing.T, setup SetupDB) {
	...
	if err := entry.StoreLogs([]*web3tool.Log{&log}); err != nil {
		t.Fatal(err)
	}
