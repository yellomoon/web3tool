...
var (
	...
	addr0B = web3tool.HexToAddress(addr0)
)
...
func TestContract_NoInput(t *testing.T) {
	...
	vals, err := c.Call("set", web3tool.Latest)
	assert.NoError(t, err)
	...
	vals, err = c1.Call("set", web3tool.Latest)
	assert.NoError(t, err)
	assert.Equal(t, vals["0"], big.NewInt(1))
}
...
func TestContract_IO(t *testing.T) {
	...
	resp, err := c.Call("setA", web3tool.Latest, addr0B, 1000)
	assert.NoError(t, err)
...
func TestContract_From(t *testing.T) {
	...
	resp, err := c.Call("example", web3tool.Latest)
	assert.NoError(t, err)
...
func TestContract_Deploy(t *testing.T) {
	...
	txn, err := DeployContract(abi, bin, []interface{}{web3tool.Address{0x1}, 1000}, WithJsonRPC(p.Eth()), WithSender(key))
	assert.NoError(t, err)
	...
	resp, err := i.Call("val_0", web3tool.Latest)
	assert.NoError(t, err)
	assert.Equal(t, resp["0"], web3tool.Address{0x1})

	resp, err = i.Call("val_1", web3tool.Latest)
	assert.NoError(t, err)
...
func TestContract_CallAtBlock(t *testing.T) {
	...
	checkVal := func(block web3tool.BlockNumber, expected *big.Int) {
	...
	// initial value is 1
	checkVal(web3tool.Latest, big.NewInt(1))
	...
	// validate the state at different blocks
	{
		...
		checkVal(web3tool.BlockNumber(receipt.BlockNumber), big.NewInt(2))

		// value at previous block is 1
		checkVal(web3tool.BlockNumber(receipt.BlockNumber-1), big.NewInt(1))
	}
}
...
func TestContract_SendValueContractCall(t *testing.T) {
	...
	found, err := client.Eth().GetBalance(addr, web3tool.Latest)
	assert.NoError(t, err)
