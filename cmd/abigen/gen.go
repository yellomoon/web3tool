...
var templateAbiStr = `// Code generated by web3tool/abigen. DO NOT EDIT.
// Hash: {{.Hash}}
// Version: {{.Version}}
...
func Deploy{{.Name}}(provider *jsonrpc.Client, from web3tool.Address, args []interface{}, opts ...contract.ContractOption) (contract.Txn, error) {
...
func New{{.Name}}(addr web3tool.Address, opts ...contract.ContractOption) *{{.Name}} {
...
func ({{$.Ptr}} *{{$.Name}}) {{funcName $key}}({{range $index, $val := tupleElems .Inputs}}{{if .Name}}{{clean .Name}}{{else}}val{{$index}}{{end}} {{arg .}}, {{end}}block ...web3tool.BlockNumber) ({{range $index, $val := tupleElems .Outputs}}retval{{$index}} {{arg .}}, {{end}}err error) {
	var out map[string]interface{}
	{{ $length := tupleLen .Outputs }}{{ if ne $length 0 }}var ok bool{{ end }}

	out, err = {{$.Ptr}}.c.Call("{{$key}}", web3tool.EncodeBlock(block...){{range $index, $val := tupleElems .Inputs}}, {{if .Name}}{{clean .Name}}{{else}}val{{$index}}{{end}}{{end}})
	if err != nil {
		return
	}
...
func ({{$.Ptr}} *{{$.Name}}) {{funcName $key}}EventSig() web3tool.Hash {
