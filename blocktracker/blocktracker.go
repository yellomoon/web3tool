...
func (t *BlockTracker) Init() (err error) {
	...
	t.once.Do(func() {
		block, err = t.provider.GetBlockByNumber(web3tool.Latest, false)
		if err != nil {
			return
		}
		if block.Number == 0 {
		...
		blocks := make([]*web3tool.Block, t.config.MaxBlockBacklog)
...
func (b *BlockTracker) LastBlocked() *web3tool.Block {
...
func (b *BlockTracker) BlocksBlocked() []*web3tool.Block {
	res := []*web3tool.Block{}
...
func (b *BlockTracker) Start() error {
	...
	// start the polling
	err := b.subscriber.Track(ctx, func(block *web3tool.Block) error {
...
func (t *BlockTracker) AddBlockLocked(block *web3tool.Block) error {
...
func (t *BlockTracker) blockAtIndex(hash web3tool.Hash) int {
...
func (t *BlockTracker) handleReconcileImpl(block *web3tool.Block) ([]*web3tool.Block, int, error) {
	...
	// The state is empty
	if len(t.blocks) == 0 {
		return []*web3tool.Block{block}, -1, nil
	}
...
func (t *BlockTracker) HandleBlockEvent(block *web3tool.Block) (*BlockEvent, error) {
	t.blocksLock.Lock()
...
func (t *BlockTracker) HandleReconcile(block *web3tool.Block) error {
...
type BlockTrackerInterface interface {
	Track(context.Context, func(block *web3tool.Block) error) error
}
...
func (k *JSONBlockTracker) Track(ctx context.Context, handle func(block *web3tool.Block) error) error {
	...
	for {
		...
		case <-time.After(k.PollInterval):
			block, err := k.provider.GetBlockByNumber(web3tool.Latest, false)
			if err != nil {
				return err
			}
...
func (s *SubscriptionBlockTracker) Track(ctx context.Context, handle func(block *web3tool.Block) error) (err error) {
	data := make(chan []byte)
	defer close(data)
