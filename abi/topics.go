...
func ParseLog(args *Type, log *web3tool.Log) (map[string]interface{}, error) {
...
func ParseTopics(args *Type, topics []web3tool.Hash) ([]interface{}, error) {
...
func ParseTopic(t *Type, topic web3tool.Hash) (interface{}, error) {
	switch t.kind {
	case KindBool:
...
func EncodeTopic(t *Type, val interface{}) (web3tool.Hash, error) {
	return encodeTopic(t, reflect.ValueOf(val))
}

func encodeTopic(t *Type, val reflect.Value) (web3tool.Hash, error) {
	switch t.kind {
	case KindBool:
	...
	return web3tool.Hash{}, fmt.Errorf("not found")
}

var topicTrue, topicFalse web3tool.Hash

func init() {
	topicTrue[31] = 1
}

func encodeTopicAddress(val reflect.Value) (res web3tool.Hash, err error) {
	var b []byte
...
func encodeTopicNum(t *Type, val reflect.Value) (res web3tool.Hash, err error) {
	var b []byte
	b, err = encodeNum(val)
	if err != nil {
		return
...
func encodeTopicBool(v reflect.Value) (res web3tool.Hash, err error) {
	if v.Kind() != reflect.Bool {
		return web3tool.Hash{}, encodeErr(v, "bool")
	}
	if v.Bool() {
