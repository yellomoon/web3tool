...
func TestEncodingBestEffort(t *testing.T) {
	...
	ethAddress := web3tool.HexToAddress(strAddress)
	...
	cases := []struct {
		...
		},
		{
			...
			[]web3tool.Address{ethAddress, ethAddress},
		},
		{
			"uint8[]",
			[]interface{}{float64(1), float64(2)},
		...
		},
		{
			...
			map[string]interface{}{
				"a": []web3tool.Address{ethAddress, ethAddress},
			},
		},
		{
...
func testTypeWithContract(t *testing.T, server *testutil.TestServer, typ *Type) error {
	...
	res, err := server.Call(&web3tool.CallMsg{
...
func TestEncodingStruct(t *testing.T) {
	...
	type Obj struct {
		A web3tool.Address `abi:"aa"`
		B *big.Int
	}
	obj := Obj{
		A: web3tool.Address{0x1},
		B: big.NewInt(1),
	}
