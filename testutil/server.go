...
import (
	...
	"github.com/yellomoon/web3tool/compiler"
...
var (
	DummyAddr = web3tool.HexToAddress("0x015f68893a39b3ba0681584387670ff8b00f4db2")
)
...
func (t *TestServer) Account(i int) web3tool.Address {
	return t.accounts[i]
}
...
func (t *TestServer) ProcessBlockWithReceipt() (*web3tool.Receipt, error) {
	receipt, err := t.SendTxn(&web3tool.Transaction{
		From:  t.accounts[0],
		To:    &DummyAddr,
		Value: big.NewInt(10),
	})
...
var emptyAddr web3tool.Address

func isEmptyAddr(w web3tool.Address) bool {
	return bytes.Equal(w[:], emptyAddr[:])
}

// Call sends a contract call
func (t *TestServer) Call(msg *web3tool.CallMsg) (string, error) {
...
func (t *TestServer) Fund(address web3tool.Address) (*web3tool.Receipt, error) {
	return t.Transfer(address, big.NewInt(1000000000000000000))
}

func (t *TestServer) Transfer(address web3tool.Address, value *big.Int) (*web3tool.Receipt, error) {
	return t.SendTxn(&web3tool.Transaction{
		From:  t.accounts[0],
		To:    &address,
		Value: value,
	})
...
func (t *TestServer) TxnTo(address web3tool.Address, method string) (*web3tool.Receipt, error) {
	return t.SendTxn(&web3tool.Transaction{
		To:    &address,
...
func (t *TestServer) SendTxn(txn *web3tool.Transaction) (*web3tool.Receipt, error) {
	if isEmptyAddr(txn.From) {
		txn.From = t.Account(0)
	}

	if txn.Type == web3tool.TransactionDynamicFee {
...
func (t *TestServer) WaitForReceipt(hash web3tool.Hash) (*web3tool.Receipt, error) {
...
func (t *TestServer) DeployContract(c *Contract) (*compiler.Artifact, web3tool.Address, error) {
	// solcContract := compile(c.Print())
	solcContract, err := c.Compile()
	if err != nil {
		return nil, web3tool.Address{}, err
	}
