...
type MockClient struct {
	lock     sync.Mutex
	num      uint64
	blockNum map[uint64]web3tool.Hash
	blocks   map[web3tool.Hash]*web3tool.Block
	logs     map[web3tool.Hash][]*web3tool.Log
	chainID  *big.Int
}
...
func (d *MockClient) GetLastBlocks(n uint64) (res []*web3tool.Block) {
	if d.num == 0 {
		return
	}
	num := n
...
func (d *MockClient) GetAllLogs() (res []*web3tool.Log) {
	if d.num == 0 {
		return
	}
...
func (d *MockClient) AddLogs(logs []*web3tool.Log) {
	if d.logs == nil {
		d.logs = map[web3tool.Hash][]*web3tool.Log{}
	}
	for _, log := range logs {
		...
		if ok {
			...
			entry = []*web3tool.Log{log}
		}
		d.logs[log.BlockHash] = entry
	}
}

func (d *MockClient) addBlocks(bb ...*web3tool.Block) {
	if d.blocks == nil {
		d.blocks = map[web3tool.Hash]*web3tool.Block{}
	}
	if d.blockNum == nil {
		d.blockNum = map[uint64]web3tool.Hash{}
	}
	for _, b := range bb {
		if b.Number > d.num {
			d.num = b.Number
...
func (d *MockClient) GetBlockByHash(hash web3tool.Hash, full bool) (*web3tool.Block, error) {
	d.lock.Lock()
	defer d.lock.Unlock()
...
func (d *MockClient) blockByNumberLock(i uint64) (*web3tool.Block, error) {
...
func (d *MockClient) GetBlockByNumber(i web3tool.BlockNumber, full bool) (*web3tool.Block, error) {
	d.lock.Lock()
	defer d.lock.Unlock()
	...
	if i < 0 {
		...
		case web3tool.Latest:
			if d.num == 0 {
				return &web3tool.Block{Number: 0}, nil
			}
...
func (d *MockClient) GetLogs(filter *web3tool.LogFilter) ([]*web3tool.Log, error) {
	d.lock.Lock()
	defer d.lock.Unlock()
...
func (m *MockBlock) GetLogs() (logs []*web3tool.Log) {
	for _, log := range m.logs {
		logs = append(logs, &web3tool.Log{Data: mustDecodeHash(log.data), BlockNumber: uint64(m.num), BlockHash: m.Hash()})
	}
	return
}
...
func encodeHash(str string) (h web3tool.Hash) {
	tmp := ""
...
func (m *MockBlock) Hash() web3tool.Hash {
	return encodeHash(m.extra + m.hash)
}

func (m *MockBlock) Block() *web3tool.Block {
	b := &web3tool.Block{
		Hash:   m.Hash(),
		Number: uint64(m.num),
	}
...
func (m *MockList) GetLogs() (res []*web3tool.Log) {
	for _, log := range *m {
...
func (m *MockList) ToBlocks() []*web3tool.Block {
	e := []*web3tool.Block{}
	for _, i := range *m {
		e = append(e, i.Block())
	}
