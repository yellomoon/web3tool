...
var (
	addressT = reflect.TypeOf(web3tool.Address{})
...
func (t *EIP712TypedData) Hash() ([]byte, error) {
	...
	return web3tool.Keccak256(res), nil
}
...
func hashStruct(primary string, types map[string][]*EIP712Type, data map[string]interface{}) ([]byte, error) {
	...
	typeHash := web3tool.Keccak256([]byte(a1))

	input := []byte{}
	input = append(input, typeHash...)
	input = append(input, a2...)

	result := web3tool.Keccak256(input)
	return result, nil
}
...
func encodeItem(typ string, types map[string][]*EIP712Type, val interface{}) ([]byte, error) {
	...
	if typ[len(typ)-1:] == "]" {
		...
		res = web3tool.Keccak256(subElem)
		...
		res = web3tool.Keccak256([]byte(valStr))
	} else if typ == "bytes" {
		...
		if valStr, ok := val.(string); ok {
			...
			res = web3tool.Keccak256(valBytes)
